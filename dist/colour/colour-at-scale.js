...
export class ColourScaleGenerator {
    ...
    computeColour(scaleNumber) {
        ...
        const normalizedLightness = clamp(rawLightness / ColourMath.lightnessScaleFactor, ColourMath.lightnessMin, ColourMath.lightnessMax);
        const hue = this.#computeScaleHue(scaleValue);
        const chroma = this.#computeScaleChroma(scaleValue);
        const coords = [hue, chroma, normalizedLightness];
    ...
    #computeScaleLightness(scaleValue) {
        ...
        const foregroundY = this.#backgroundY > ColourMath.backgroundLightnessThreshold
