...
const YtoL = (Y: number) => (Y <= 0.008_856_451_6 ? Y * 903.296_296_2 : 116 * Math.pow(Y, 1 / 3) - 16)

const toe = (l: number) => 0.5 * (k3 * l - k1 + Math.sqrt((k3 * l - k1) * (k3 * l - k1) + 4 * k2 * k3 * l))

const normalizeScaleNumber = (scaleNumber: number, maxScaleNumber: number) => scaleNumber / maxScaleNumber

// hue, chroma, and lightness functions
const computeScaleHue = (scaleValue: number, baseHue: number) => baseHue + 5 * (1 - scaleValue)

const computeScaleChroma = (scaleValue: number, minChroma: number, maxChroma: number) => {
...
export const computeColorAtScaleNumber = (
    scaleNumber: number,
